...
| 의존성 | `_repo/requirements.txt` (최소 버전 고정) |
...
| 산출물 | 경로 |
|---|---|
| 학습 결과(시드별 정확도·혼동행렬) | `_results/artifacts/<run>/eval_results.json`, `history.json` |
| 기준선 결과 | `_results/artifacts/feature_{logreg,rbf_svm}.json` |
| 스트레스 결과 | `_results/stress/{snr_sweep,aspect_sweep,failure_cases}.json` |
| 그림(7종, PNG+PDF) | `_results/figures/` |
| 결과 요약 | `_results/RESULTS.md`, `_results/STRESS_RESULTS.md` |
...
- 보고서 PDF: `_report/202650765_변건_W14.pdf`
- 코드 제출: 위 저장소를 fork 후 본인 추가 스크립트(`_results/stress/run_stress.py`, `collect_failures.py`, `_results/gen_figures.py`)를 포함해 링크하거나, `projects/p02_resnet18_har/` + 위 스크립트 + 본 README를 zip으로 제출.
